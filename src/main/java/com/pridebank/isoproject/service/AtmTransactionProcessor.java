...
public class AtmTransactionProcessor {
    ...
    public IsoMessage processTransaction(IsoMessage isoRequest) {
        ...
        try {
            if (safeHasField(isoRequest, 127)) {
                ...
                if (val127 == null) {
                    ...
                    log.info("Diagnostic: Field 127 is raw bytes (base64) length={} data={}", ((byte[]) val127).length, Base64.getEncoder().encodeToString((byte[]) val127));
                } else {
        ...
        try {
            ...
            log.warn("Failed removing forbidden 127 subfields from template; leaving 127 intact", ex);
        }
        ...
        try {
            ...
            IsoMessage finalResp = buildResponseContainingOnlyAllowed(responseMti, allowed, isoRequest, esbIsoResp, responseTemplate);
            ...
            try {
                ...
                if ("00".equals(rc)) {
                    if (txType.equals("01") || txType.equals("21") || txType.equals("31") || txType.equals("00") || txType.equals("02")) {
            ...
            // Response-side 127 visibility
            try {
                if (safeHasField(finalResp, 127)) {
                    Object v127 = safeGetObjectValue(finalResp, 127);
                    if (v127 instanceof byte[]) {
                        IsoMessage nestedLog = decode127ToIsoMessage((byte[]) v127);
                        Map<String, Object> sub = (nestedLog != null) ? buildFieldMapForLog(nestedLog) : Map.of();
                        log.info("Final response 127.* subfields (post-prune): {}", sub);
                    } else if (v127 instanceof IsoMessage) {
                        Map<String, Object> sub = buildFieldMapForLog((IsoMessage) v127);
                        log.info("Final response 127.* subfields (nested IsoMessage): {}", sub);
                    } else {
                        log.info("Final response 127 value: {}", safeRenderValue(v127));
                    }
                } else {
                    log.info("Final response has no 127 field");
                }
            } catch (Exception ignore) {
            }
    ...
    private IsoMessage clone127WithoutForbidden(IsoMessage nested) {
        ...
        for (int j = 1; j <= 128; j++) {
            try {
                ...
                try {
                    copy.setValue(j, sval, IsoType.LLLVAR, Math.min(sval.length(), 999));
                } catch (Throwable ignore) {
                    try {
    ...
    private IsoMessage buildResponseContainingOnlyAllowed(
        ...
        for (Integer f : allowed) {
            ...
            try {
                if (f == 127) {
                    log.info("Processing allowed field {}", f);
                    Object val127 = null;
                    ...
                    if (val127 == null) continue;

                    try {
                        if (val127 instanceof IsoMessage nested) {
                            byte[] bytes = packNested127WithSubPackager(nested);
                            if (bytes == null || bytes.length == 0) {
                                bytes = nested.writeData();
                            }
                            byte[] pruned = prune127Bytes(bytes);
                            if (pruned != null && pruned.length > 0) {
                                resp.setValue(127, pruned, IsoType.BINARY, pruned.length);
                            } else {
                                try {
                                    Method unset = resp.getClass().getMethod("unset", int.class);
                                    unset.invoke(resp, 127);
                                } catch (Exception ignore) {
                                }
                            }
                            continue;
                        ...
                        if (val127 instanceof byte[]) {
                            byte[] pruned = prune127Bytes((byte[]) val127);
                            resp.setValue(127, pruned, IsoType.BINARY, pruned.length);
                            continue;
                        }
                        if (val127 instanceof String s) {
                            String trimmed = s.trim();
                            if (looksJsonObject(trimmed)) {
                                String edited = stripJsonKeys(trimmed, Set.of("22", "25"));
                                resp.setValue(127, edited, IsoType.LLLVAR, Math.min(edited.length(), 999));
                            } else {
                                resp.setValue(127, trimmed, IsoType.LLLVAR, Math.min(trimmed.length(), 999));
                                log.debug("127 string mirrored (non-JSON)");
                            }
                            continue;
                        }
                        ...
                        log.debug("Failed mirroring 127 value: {}", ex.getMessage());
                        continue;
                    }
                }
    ...
    private Object safeRenderValue(Object v) {
        try {
            ...
            if (v instanceof IsoMessage nested) return buildFieldMapForLog(nested);
    ...
    private void removeForbidden127Subfields(IsoMessage msg) {
        ...
        try {
            ...
            if (val127 instanceof byte[]) {
                byte[] pruned = prune127Bytes((byte[]) val127);
                try {
                    msg.setValue(127, pruned, IsoType.BINARY, pruned.length);
                } catch (Exception setEx) {
                    log.debug("Failed to set pruned 127 bytes: {}", setEx.getMessage());
                }
                return;
            }
            ...
            if (val127 instanceof String) {
                ...
                if (looksJsonObject(s)) {
                    String edited = stripJsonKeys(s, Set.of("22", "25"));
                    try {
                        try {
                    ...
                    log.debug("127 string not JSON; preserving as-is");
                    return;
                }
            }
        } catch (Exception e) {
    ...
    private void pruneResponseFields(IsoMessage msg, Set<Integer> allowed) {
        ...
        for (int f = 1; f <= 128; f++) {
            try {
                ...
                if (!removed) {
                    log.debug("Unable to remove field {} from response. Last error: {}", f, lastEx.getMessage());
                }
            } catch (Exception ex) {
    ...
    private ISOBasePackager getF127SubPackager() {
        if (jposPackager == null) return null;
        try {
            ISOFieldPackager fp = jposPackager.getFieldPackager(127);
            if (fp instanceof ISOMsgFieldPackager) {
                Field packagerField = ISOMsgFieldPackager.class.getDeclaredField("packager");
                packagerField.setAccessible(true);
                Object sub = packagerField.get(fp);
                if (sub instanceof ISOBasePackager) {
                    return (ISOBasePackager) sub;
                }
            }
        } catch (Exception e) {
            log.debug("getF127SubPackager reflection failed: {}", e.getMessage());
        }
        return null;
    }

    private IsoMessage decode127ToIsoMessage(byte[] bytes) {
        try {
            if (bytes == null || bytes.length == 0) return null;
            ISOBasePackager sub = getF127SubPackager();
            if (sub == null) {
                log.debug("No 127 sub-packager available; skipping 127 decode");
                return null;
            }
            ISOMsg j = new ISOMsg();
            j.setPackager(sub);
            ...
            IsoMessage nested = new IsoMessage();
            for (int i = 1; i <= 128; i++) {
                try {
                    if (!j.hasField(i)) continue;
                    byte[] b = j.getBytes(i);
                    if (b != null) {
                        nested.setValue(i, b, IsoType.BINARY, b.length);
                        continue;
                    }
                    String s = j.getString(i);
                    if (s != null) {
                        nested.setValue(i, s, IsoType.LLLVAR, Math.min(s.length(), 999));
                    }
                } catch (Exception ignore) {
                }
            }
            ...
            log.debug("decode127ToIsoMessage failed: {}", t.getMessage());
            return null;
        }
    }

    private byte[] packNested127WithSubPackager(IsoMessage nested) {
        ISOBasePackager sub = getF127SubPackager();
        if (sub == null || nested == null) return null;
        try {
            ISOMsg j = new ISOMsg();
            j.setPackager(sub);
            for (int i = 1; i <= 128; i++) {
                try {
                    if (!safeHasField(nested, i)) continue;
                    Object v = safeGetObjectValue(nested, i);
                    if (v == null) continue;
                    if (v instanceof byte[]) j.set(i, (byte[]) v);
                    else j.set(i, v.toString());
                } catch (Exception ignore) {
                }
            }
            return j.pack();
        } catch (Exception e) {
            log.debug("packNested127WithSubPackager failed: {}", e.getMessage());
            return null;
        }
    }

    private byte[] prune127Bytes(byte[] in) {
        if (in == null || in.length == 0) return in;
        ISOBasePackager sub = getF127SubPackager();
        if (sub == null) return in;
        try {
            ISOMsg j = new ISOMsg();
            j.setPackager(sub);
            j.unpack(in);
            j.unset(22); // STRUCTURED DATA
            j.unset(25); // ICC DATA
            return j.pack();
        } catch (Exception e) {
            log.debug("prune127Bytes failed: {}", e.getMessage());
            return in;
        }
    }
    ...
    private static boolean looksJsonObject(String s) {
        ...
        return Pattern.compile("\"\\d+\"\\s*:\\s*").matcher(t).find();
    }

    private static String stripJsonKeys(String json, Set<String> keys) {
        String edited = json;
        for (String k : keys) {
            edited = edited.replaceAll("\"" + Pattern.quote(k) + "\"\\s*:\\s*\"[^\"]*\"\\s*,?", "");
            edited = edited.replaceAll("\"" + Pattern.quote(k) + "\"\\s*:\\s*\\{[^}]*}\\s*,?", "");
            edited = edited.replaceAll("\"" + Pattern.quote(k) + "\"\\s*:\\s*\\[[^]]*]\\s*,?", "");
        }
